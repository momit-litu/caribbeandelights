        ...
        <div class="x_content">
            ...
            <div class="x_panel">
                <div class="row advance_search_div alert alert-warning">
                    <div class="row">
                        <label class="control-label col-md-2 col-sm-4 col-xs-4" style="text-align:right">Category</label>
                        <input class="control-label col-md-2 col-sm-4 col-xs-4" type="text" name="category_name" id="category_name">
                        <input type="hidden" name="category_id" id="category_id" value="0">
                        <label class="control-label col-md-2 col-sm-4 col-xs-4" style="text-align:right">Active</label>
...
<script>
    ...
    $(document).ready(function () {
        ...
        load_item_grid = function load_item_grid(search_txt){
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if($.trim(search_txt) == "Print"){
                        ...
                        html +='<button class="no-print" onclick="window.print()">Print</button><div width="100%"  style="text-align:center"><img src="'+project_url+'<?php echo $logo; ?>" width="80"/></div><h2 style="text-align:center">Karibbean Delight</h2><h4 style="text-align:center">Items Report</h4><table width="100%"><tr><th width="60%" style="text-align:left"><small>'+serach_areas+'</small></th><th width="40%"  style="text-align:right"><small>Printed By: '+user_name+', Date:'+todate+'</small></th></tr></table>';

                        if(!jQuery.isEmptyObject(data.records)){

                            html +='<table width="100%" cellpadding="10" border="1px" style="margin-top:10px;border-collapse:collapse"><thead><tr>' +
                                '<th style="text-align:left">Id</th><th style="text-align:left">Item Name</th>' +
                                '<th width="40%" style="text-align:left">Item Details</th><th style="text-align:left">Category Name</th>' +
                                '<th style="text-align:right">Item Price</th><th style="text-align:center">Is Combo</th>' +
                                '<th style="text-align:center">Hot Item</th></tr></thead><tbody>';
                            $.each(data.records, function(i,data){
                                //alert(data)
                                html += "<tr>";
                                html +="<td style='text-align:left'>"+data.item_id+"</td>";
                                html +="<td style='text-align:left; text-transform:capitalize'>"+data.item_name+"</td>";
                                html +="<td style='text-align:left; text-transform:capitalize'>"+data.details+"</td>";
                                html +="<td style='text-align:left; text-transform:capitalize'>"+data.category_name+"</td>";
                                html +="<td style='text-align:right;'>$"+data.price+"</td>";
                                html +="<td style='text-align:center'>"+data.is_combo+"</td>";
                                html +="<td style='text-align:center'>"+data.hot_item+"</td>";
                                html += '</tr>';
                            });
                            html +="</tbody></table>"
                        }
                        ...
                        WinId = window.open("", "Item Report","width=850,height=800,left=50,toolbar=no,menubar=YES,status=YES,resizable=YES,location=no,directories=no, scrollbars=YES");
                        WinId.document.open();
