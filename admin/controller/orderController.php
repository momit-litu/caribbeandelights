       ...
       // if(isset($ad_delivery_date))  			$condition  .=" and delivery_date = cast(getdate() as Date)";
        ...
        $countsql = "SELECT count(order_id)
					FROM(
						...
						case m.payment_method when 1 then 'Cash' when 2 then 'Loyalty Point' when 3 then 'Paypal' when 6 then 'Authorize' else 'Square'  end payment_method
						FROM order_master m
...
//var_dump($status_id);
        ...
        if($return){
            ...
            //$subject = "Your order no #$order_id has been $subject by Karibbean Delight";
            //$body 	 = "Dear ".$email['full_name']."<br>Your order". $order_id." has been". $subject." by Karibbean Delight. <br>Thanks<br>Karibbean Delight";


            if($status_id == 2){
                $subject = "Your order no #".$order_id." has been Rejected/Canceled by Karibbean Delight";
                ...
                $subject = $order_id." Order ".$email['full_name']." Confirmation from Karibbean Delight ";
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order". $order_id." has been Received by Karibbean Delight. </p><p>Thanks</p><p>Karibbean Delight</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
                ...
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order ". $order_id." has been Ready for  pick. </p><p>Thanks</p><p>Karibbean Delight</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
            }
            else if($status_id == 5){
                //$details = " Your Order (".$order_id.") is Delivered ";
                $subject = "Your order no #".$order_id." has been picked up.";
                $body 	 = "<p>Dear ".$email['full_name']."</p><p>Your order". $order_id." has been picked up </p><p>Thanks</p><p>Karibbean Delight</p>";
                $dbClass->sendMail($email['email'], $subject, $body);
            }
    ...
    case "get_order_details_by_invoice":
        ...
        if($group_order_id['group_order_id']==0){
            $sql = "SELECT m.order_id, m.customer_id, ifnull(c.full_name,'Instance Order') customer_name, d.item_id, ifnull(c.contact_no,'') customer_contact_no, ifnull(c.email,'') email, m.order_id,
                ...
                case payment_method when 1 then 'Cash' when 3 then 'Paypal' when 6 then 'Authorize' when 2 then 'Loyalty'  else 'Square'  end payment_method
                FROM order_master m
                LEFT JOIN order_details d ON d.order_id = m.order_id
                LEFT JOIN customer_infos c ON c.customer_id = m.customer_id
                LEFT JOIN items p ON p.item_id = d.item_id
            ...
            $sql = "SELECT ci.full_name,ci.customer_id, ci.email as email, go.discount_amount, ci.contact_no as mobile, gi.name,go.payment_status,
                    ...
                    case go.payment_method when 1 then 'Cash' when 3 then 'Paypal' when 6 then 'Authorize' when 2 then 'Loyalty'  else 'Square'  end payment_method,
    ...
    case "get_group_order_details":
        ...
        $sql = "SELECT ci.full_name, ci.address as c_address, ci.contact_no as mobile, gi.name, go.order_id as group_order_id, go.order_date, go.delivery_date, go.total_order_amt, go.notification_time,  cp.c_type, cp.amount as cupon_amount, cp.min_order_amount, go.order_status as status, go.invoice_no,go.tips,go.total_paid_amount,
                    ...
                    case go.payment_method when 1 then 'Cash On Delivery' when 2 then 'Loyalty Payment' when 3 then 'Paypal' when 6 then 'Authorize' when 4 then 'Square' else 'Not Defined' end payment_method
                    from group_order go
                    LEFT JOIN groups_info gi ON gi.id = go.group_id
                    LEFT JOIN(
...
//var_dump($status_id);
        ...
        if($return){
            ...
            $body 	 = "<p>Dear ".$email['full_name'].",</p><p>We apologise that we  are not able to fulfill your order ". $order_id."
                A refund of ".$id['total_paid_amount']." has been issued. </p><p>Thank you for patronage.</p><p>Sincerley.</p><p>Karibbean Delight</p>";
            $dbClass->sendMail($email['email'], $subject, $body);
