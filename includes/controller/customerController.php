	...
	$query="select email, username, customer_id from customer_infos WHERE  email='".$forget_email."'  and status=1";
		...
		if($stmt -> rowCount()>0){
            ...
            if(isset($is_app)){
                ...
                $body = 'Dear '.$username.',<br><p>We are processing your password reset request, Your new password is: "'. $pass_key.'" </p><p>Please reset your password right after login.</p><p><br> Thank,</p><br>Karibbean Delight';
                $columns_value = array(
                ...
                $body = 'Dear '.$username.',<br><p>We are processing your password reset request, Please <a href="'.$pass_reset_url.'">click here</a> to login your account and update password.<b></p><p>Keep Old password field empty while reset your password through this link.</b></p><p><br> Thank,</p><br>Karibbean Delight';
                $status = 1;
            }
			...
			$subject = "Password Reset Request from Karibbean Delight";
...
if($q=="registration"){
    ...
    if($return) {
        ...
        $body 	 = 'Dear '.$cust_name.'<br><p>You have been successfully registered to Karibbean Delight. 
            To Select Menus please visit <a href="'.$web_url["website_url"].'"> Karibbean Delight</a>.</p><p><br><br>
            Best Regards</p><p><b>Karibbean Delight</b></p><br>Note: Please Do not reply this email.';
