...
class orderConfirm {
        ...
        if($customer_email){
           ...
           // $stmt = $conn->prepare($sql);
			...
			if($paid_status!='Paid' && ($pay_methode==3 || $pay_methode==4)){
				$subject = "#". $invoice_no." Order Confirmation from Karibbean Delight";
				...
				$subject = "#". $invoice_no." Order Paid Confirmation from Karibbean Delight";
            ...
            $body .= "<link rel='stylesheet' type='text/css' href='".$web_url["website_url"]."plugin/bootstrap/bootstrap.css'>
                        <div id='order-div'>
                            <div class='title text-center' style='text-align: center'>
                                <img src='".$web_url["website_url"]."admin/images/banner/logo.png' alt='' style='alignment: center'>
    ...
    function afterPayment($order_id){
       ...
       // $this->dbClass->print_arrays($customer);
    ...
    function paymentAdd($txnid, $payment_amount, $payment_status, $method, $itemid){
        ...
        try {
            $column_value = array(
                ...
                'payment_method'=> ($method==6)?$method:($method==0 ? 3 : 4),
            );
            $condition_array = array(
                'invoice_no' =>$item_number
            );
			//var_dump($column_value, $condition_array);
