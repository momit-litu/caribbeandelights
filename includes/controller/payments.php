...
//$orderConfirm->afterPayment('KD082004861');
//$orderConfirm->afterPayment('KD082004737');

$paypal_email = $dbClass->getDescription("paypal_email");
$project_url = $dbClass->getDescription('website_url');

//$paypal_sandbox = 'sb-edrba1353003@business.example.com'; // BB credentials
$paypal_sandbox = 'sb-ugriq5380773@business.example.com';
extract($_POST);
...
if (!isset($_POST["txn_id"]) && !isset($_POST["txn_type"])) {
    ...
    else if (strpos($item_name, 'KDG') !== false) {
        $paypalConfig = [
            'email' => $enableSandbox ? $paypal_sandbox : $paypal_email,
            'return_url' => $next_url,
            'cancel_url' => $project_url.'account',
    ...
    //$orderConfirm->afterPayment('KD082004862');
    ...
    //$orderConfirm->afterPayment('KD082004864');

    if (checkTxnid($data['txn_id'])) {
        $item_number= trim(preg_replace('/\s+/', '', $data['item_number']));
        //$orderConfirm->afterPayment('KD082004864');z
        try {
