...
switch ($q){
    ...
    case "make_payment":
        ...
        if($payment_method==2){
            ...
            if (strpos($data['item_number'], 'KDG') !== false) {
...
//this portion will remove while final submission......
    ...
    case "test_mail":
        ...
        $sql = "SELECT m.order_id, m.customer_id, 
                    ...
                    case payment_method when 1 then 'cash' when 2 then 'loyalty_point' when 3 then 'Paypal' when 6 then 'Authorize'  else 'Square'  end payment_method
                    FROM order_master m
                    LEFT JOIN order_details d ON d.order_id = m.order_id
                    LEFT JOIN customer_infos c ON c.customer_id = m.customer_id
                    LEFT JOIN items p ON p.item_id = d.item_id
